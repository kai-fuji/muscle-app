...
export async function getDb() {
  ...
  if (tursoUrl && tursoToken) {
    ...
    // Tursoでは initializeTables() を実行しない（既に存在する前提）
...
async function initializeTables() {
  ...
  // training高速化
  await db.execute(`
    CREATE INDEX IF NOT EXISTS idx_training_date_time
    ON training(date, time)
  `)

  // nutrition高速化
  await db.execute(`
    CREATE INDEX IF NOT EXISTS idx_nutrition_date_time
    ON nutrition(date, time)
  `)
...
export async function getAllBodyData() {
  ...
  const result = await db.execute(
    'SELECT * FROM body_data ORDER BY date DESC'
  )
  return result.rows
}
...
export async function getAllNutritionData() {
  const db = await getDb()
  const result = await db.execute(
    'SELECT * FROM nutrition ORDER BY date DESC, time DESC'
  )
  return result.rows
}
...
export async function updateNutritionData(date, time, meal, calories, protein, fat, carbs, fiber) {
  ...
  await db.execute({
    sql: `
      UPDATE nutrition
      SET meal = ?, calories = ?, protein = ?, fat = ?, carbs = ?, fiber = ?
      WHERE date = ? AND time = ?
    `,
...
export async function updateTrainingData(date, time, exercise, sets, reps, weight) {
  ...
  await db.execute({
    sql: `
      ...
      WHERE date = ? AND time = ? AND exercise = ?
    `,
