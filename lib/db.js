...
async function initializeTables() {
  ...
  try {
    await db.execute('ALTER TABLE training ADD COLUMN negative INTEGER DEFAULT 3')
    console.log('✅ Added negative column to training table')
  } catch (e) {
    // カラムが既に存在する場合は無視
    if (!e.message.includes('duplicate column')) {
      console.log('ℹ️ negative column already exists or error:', e.message)
    }
  }

  try {
    await db.execute('ALTER TABLE training ADD COLUMN interval_seconds INTEGER DEFAULT 60')
    console.log('✅ Added interval_seconds column to training table')
  } catch (e) {
    // カラムが既に存在する場合は無視
    if (!e.message.includes('duplicate column')) {
      console.log('ℹ️ interval_seconds column already exists or error:', e.message)
    }
  }
