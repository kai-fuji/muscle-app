...
async function initializeTables() {
  ...
  await db.execute(`
    CREATE TABLE IF NOT EXISTS inbody_data (
      ...
      inbody_score REAL,
      right_arm_muscle REAL,
      left_arm_muscle REAL,
      trunk_muscle REAL,
      right_leg_muscle REAL,
      left_leg_muscle REAL,
      right_arm_fat REAL,
      left_arm_fat REAL,
      trunk_fat REAL,
      right_leg_fat REAL,
...
export async function getAllInbodyData() {
  const db = await getDb()
  const result = await db.execute('SELECT * FROM inbody_data ORDER BY date DESC')
  return result.rows
}

export async function addInbodyData(data) {
  const db = await getDb()
  const {
    date, weight, body_fat_percentage, skeletal_muscle_mass,
    muscle_mass, body_fat_mass, basal_metabolic_rate, inbody_score,
  ...
  await db.execute({
    sql: `INSERT OR REPLACE INTO inbody_data (
      date, weight, body_fat_percentage, skeletal_muscle_mass,
      muscle_mass, body_fat_mass, basal_metabolic_rate, inbody_score,
      right_arm_muscle, left_arm_muscle, trunk_muscle, right_leg_muscle, left_leg_muscle,
      right_arm_fat, left_arm_fat, trunk_fat, right_leg_fat, left_leg_fat
...
export async function deleteInbodyData(date) {
  const db = await getDb()
  await db.execute({
    sql: 'DELETE FROM inbody_data WHERE date = ?',
    args: [date]
  })
}
